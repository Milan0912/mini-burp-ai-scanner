 ...
 * AttackEngine.js
 * ===============
 * ONE responsibility: Execute a request and return a structured response.
 * Handles encoding and mutation logic.
 */

async function execute(reqCtx, mutation = null) {
    console.log(`[AttackEngine] Executing ${reqCtx.method} on ${reqCtx.url}`);
    try {
        const mutated = mutation ? mutate(reqCtx, mutation.param, mutation.payload, mutation.source) : reqCtx;
        ...
        // Task 1: Hard Timeout - Increased to 30 seconds
        ...
        if (hi === -1) {
            // Task 5: Still analyze if we got something
            return {
                status: 0,
        ...
        return { 
            status, 
            body: bodyPart, 
            length: Buffer.byteLength(bodyPart), 
            elapsed, 
            headers, 
        ...
        console.error(`[AttackEngine] Request failed: ${e.message}`);
        // Log errors to avoid failing silently
        return null; 
    }
}

function mutate(reqCtx, param, payload, source) {
